...
DB_CONTAINER_NAME="praxis-mysql"
...
docker run --name $DB_CONTAINER_NAME -e MYSQL_ROOT_PASSWORD=$DB_PASSWORD -e MYSQL_DATABASE=praxis -d -p 3306:3306 docker.io/mysql # Comente se BD já criado
